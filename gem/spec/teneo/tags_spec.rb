...
RSpec.describe 'Format Library' do
  it 'multiple formats can be tagged by the same tag' do
    tag = Teneo::FormatLibrary::Tag['be.libis.teneo:BMP']
  ...
  it 'tags can be tagged by other tags' do
    ...
    expect(tag.ancestors_hash.keys).to include('be.libis.teneo:IMAGE')
    expect(tag.ancestors_ds.map { |t| t[:tag] }).to include('be.libis.teneo:IMAGE')
  end

  it 'tagged tags can be found' do
    tag = Teneo::FormatLibrary::Tag['be.libis.teneo:IMAGE']
    expect(tag.descendants_hash.size).to be >= 1
    expect(tag.descendants_hash.size).to eq(tag.descendants_ds.count)
    expect(tag.descendants_hash.keys).to include('be.libis.teneo:BMP')
    expect(tag.descendants_ds.map { |t| t[:tag] }).to include('be.libis.teneo:BMP')
  end
